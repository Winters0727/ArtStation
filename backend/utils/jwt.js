...
const verifyToken = function(inputToken, inputRefreshToken) {
    try {
        ...
        try {
            ...
            const { _id, userEmail, userNickname, userProfilePic } = result;
            const payload = { "_id" : _id, "userEmail" : userEmail, "userNickname" : userNickname, "userProfilePic" : userProfilePic}; // 리프레시 토큰 결과로 페이로드를 만들고
